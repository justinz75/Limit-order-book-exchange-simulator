...
int main() {
    ...
    //an empty name gives a writer that records nothing
    DataWriter writer("");
    ...
    //about a tenth of the orders should be market orders
    ...
    //a default config must give exactly the original model
    ...
    //informed traders should find mispricings to trade on
    ...
    //the maker's position must equal what it bought less what it sold
    ...
    //the maker only rests, so it should never buy above or sell below the mid it saw
    ...
    //the markout at zero must match the spread earned per unit, worked out separately
